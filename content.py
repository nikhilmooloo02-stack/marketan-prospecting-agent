...
if APP_PASSWORD:
    ...
    if not st.session_state.authenticated:
        st.title("MarkeTan Content Library")
...
def load_content():
    ...
    rows = conn.execute(
        """
        SELECT id, content_type, platform, body, visual_brief, status, created_at
        FROM content
...
for post in filtered:
    with st.container(border=True):
        ...
        with left:
            ...
            if post.get("visual_brief"):
                with st.expander("🎨 Visual brief (for Canva)"):
                    st.write(post["visual_brief"])
